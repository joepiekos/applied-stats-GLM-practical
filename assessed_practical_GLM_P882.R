...
### mention Wald tests
### also chi squared stuff for saturated model? (is it still valid for poisson with low counts? maybe only breaks for saturated model).
#q3 - diagnostics
...
fittedval.deviance.plot <- ggplot(residual.fittedval) + geom_point(aes(x=fitted.eta,y=deviance.residuals)) + theme(axis.title = element_text(size = 16),axis.text=element_text(size=16))#pretty bad, as expected for poisson with low count models
...
model.coefficients <- exp(summary(reduced.model)$coefficients[c(1,3,4,5),1])
model.coefficients <- rownames_to_column(as.data.frame(model.coefficients),var = "Variable") %>% as_tibble() %>% rename(Effect = model.coefficients) %>% add_row(Variable = c("age|female = 0","age|female = 1"), Effect = exp(c(summary(reduced.model)$coefficients[2,1], summary(reduced.model)$coefficients[2,1] + summary(reduced.model)$coefficients[7,1])))
model.coefficients <- model.coefficients %>% add_column(Std_Error = sqrt(c(model.covmatrix[1,1],model.covmatrix[3,3],model.covmatrix[4,4],model.covmatrix[5,5],model.covmatrix[2,2],model.covmatrix[2,2]+model.covmatrix[7,7]+(2*model.covmatrix[7,2]))))
model.coefficients <- model.coefficients %>% mutate(Lower = exp(log(Effect) - 1.96*Std_Error), Upper = exp(log(Effect) + 1.96*Std_Error))
